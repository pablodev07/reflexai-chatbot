...
const handleSubmit = async () => {
  ...
  setTimeout(async () => {
    ...
    setCurrentResponseIndex((prevIndex) => {
      ...
      if (nextIndex === chatbotResponses.length) {
        endChat();
      }
...
const startChatTimeout = () => {
  ...
  chatTimeoutRef.current = timeout;
};
...
const endChat = () => {
  ...
  setMessages((prevMessages) => [
    ...prevMessages,
    { sender: 'system', text: 'Chat closed.', timestamp: new Date().toISOString() },
  ]);
    ...
    <main className="chatContainer flex flex-col lg:flex-row h-screen">
      ...
      <section className="relative flex flex-1 flex-col overflow-hidden bg-smoothgray">
        ...
        <div className="inputContainer sticky bottom-0 mt-4 mx-5 mb-5">
          <textarea value={userInput} onChange={(e) => setUserInput(e.target.value)} onKeyDown={handleKeyDown} className="chatInput w-full rounded-[24px] min-h-[100px] p-5 border-2 border-gray-200 outline-primary overflow-y-auto resize-none" placeholder="Enter your message" />
